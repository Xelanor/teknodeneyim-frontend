...
export const likeComment = (commentId, userId) => dispatch => {
  axios.post(`${process.env.REACT_APP_API_URL}/comments/${commentId}/like`, { userId })
    .then(res => {
...
export const savePost = (postId, userId) => async dispatch => {
  await axios.post(`${process.env.REACT_APP_API_URL}/posts/${postId}/save`, { userId })
    .then(async res => {
      if (res.status === 200) {
        await axios.post(`${process.env.REACT_APP_API_URL}/users/user-save-post`, { userId, postId })
