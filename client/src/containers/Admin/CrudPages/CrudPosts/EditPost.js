...
class EditPost extends Component {
  ...
  async componentDidMount() {
    await axios
      .get(
        `${process.env.REACT_APP_PROXY}/posts/post/${this.props.match.params.id}`
      )
  ...
  onFormSubmitHandler = (e) => {
    ...
    axios
      .post(`${process.env.REACT_APP_PROXY}/admin/posts/edit`, post)
        ...
        <div className="w-full max-w-sm items-center mx-auto">
          {this.state.post ? (
            <form onSubmit={this.onFormSubmitHandler}>
              ...
              <button
                className="bg-tekno hover:bg-tekno text-white font-bold py-2 px-6 rounded focus:outline-none focus:shadow-outline"
                type="submit"
              >
                Konuyu Düzenle
              </button>
